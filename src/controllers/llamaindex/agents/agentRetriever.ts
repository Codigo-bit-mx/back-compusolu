...
import { OpenAI, storageContextFromDefaults, Document, VectorStoreIndex, Settings, OpenAIEmbedding } from "llamaindex";
import { Context,
         StartEvent,
         StopEvent,
         Workflow,
...
// const llm = new OpenAI({ model: , temperature: 1 });
Settings.llm = new OpenAI({ model: "gpt-4o", temperature: 1});
...
export class RefinementEvent extends WorkflowEvent<{ ref: any }> {}
...
const indexChunck = async (context: Context, ev: StartEvent) => {
    // indexamiento del conocimiento 
    context.set("specification", ev.data.input);
    const spec = context.get("specification");

    context.writeEventToStream(
        new MessageEvent({
        msg: `Razona correctamente la informacion, necesitamos indexar el conocimiento para poder responder a la pregunta: ${spec}, `,
        }),
    );
    ...
    operativo, contable y contagio, entre otros.`; 
    ...
    const document = [
        new Document({ text: training }),
    ] 

    const vectorIndex = await VectorStoreIndex.fromDocuments(document, { storageContext });
    ...
    const result = await queryEngine.query({
     ...
     console.log("index", result.message.content);

    return new RefinementEvent({ref: result.message.content});

}

const refinement = async (context: Context, ev:RefinementEvent) => {

    // leer la caprta con el indexamiento y veriricar si la respuesta es la mas correcta posible 
    const result = ev.data.ref;
    console.log("entro", result);
    //abrea revisores que puedan ver el resultado y entregar la mas correcta posible 
    ...
    if (numberReviews >= MAX_REVIEWS) {
        context.writeEventToStream(
            new MessageEvent({
                msg: `Se han realizado ${MAX_REVIEWS} revisiones. Esta es la respuesta final: ${result}`,
            }),   
        )    
    ...
    context.writeEventToStream(
        new MessageEvent({ msg: `carga el indexamiento correctamente y verifica correctamente que la respuesta sea la correcta
                - esta es la respuesta anterior: ${result}` }),
    );

    let loadStorage = await storageContextFromDefaults({ persistDir: './storage' });
        
    const loadIndexStorage = await VectorStoreIndex.init({ 
          storageContext: loadStorage
      });

    const queryEngineIndex = loadIndexStorage.asQueryEngine();

    const response = await queryEngineIndex.query({
        query: result,
    });

    console.log("refina", response.message.content);
    return new RefinementEvent({ref: response.message.content});

}


const codeAgent = new Workflow({ validate: true })
codeAgent.addStep(StartEvent, indexChunck, {outputs: RefinementEvent})
codeAgent.addStep(RefinementEvent, refinement, {outputs: RefinementEvent})


export const agentRetriever = async (req: Request, res: Response) => {
