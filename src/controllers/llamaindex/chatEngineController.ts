...
export const chatEngineController = async (req: Request, res: Response) => {
  ...
  try {
        ...
        const document = new Document({ text: training }); // texto de entrenamiento
        ...
        const index = await VectorStoreIndex.fromDocuments([document], {
         serviceContext,
       }); //índice que almacena los nodos solo según sus incrustaciones de vectores
       
        const retriever = index.asRetriever(); //recuperador de datos

        const chatEngine = new ContextChatEngine({ //motor de chat
        ...
        const prompt = `actua como un chatbot, encuentra la siguiente pregunta ${query}`; //se puede integrar la pregunta al prompt para obtener una respuesta mas especifica 
        
        const stream = await chatEngine.chat({ message: prompt, stream: true }); // uso del motor de chat para obtener la respuesta
        console.log();
